...
void create()
{
    ...
    state = SLEEPING;
    set_heart_beat(30);//每次心跳的秒数通常为2秒，30次心跳即30*2=60秒，每60秒执行一次heart_beat()。
}
...
int join_competition(object ob)
{
    ...
    if (ob->query("combat_exp") < 800000 && !ob->query_temp("team_fight"))
...
private void send_invite_message()
{
    ...
    message("vision", HIR "你收到了乌老大撒下的武林盟帖，邀请你参加这次" + tlist[selected]["name"] + "，有没有兴趣去显显身手？\n" NOR,
            obs);
}
...
int start_by_others(int n)
{
    ...
    if (n < 0 || n >= sizeof(tlist))
        return notify_fail("没有这个等级的比赛(0-" + (sizeof(tlist) - 1) + ")。\n");
    selected = n;
    change_state(GET_READY);
    return 1;
}
