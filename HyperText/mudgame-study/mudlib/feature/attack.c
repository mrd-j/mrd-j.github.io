...
void fight_ob(object ob)
{
    ...
    env = environment();
    if (environment(ob) != env || env->query("no_fight"))
        return;
...
void kill_ob(object ob)
{
    ...
    {
        if (!gob || gob == me || environment(gob) != environment())
            continue;
        ...
        tell_object(gob, HIR + ob->name(1) + "受到攻击，你挺身而出，加入战团！\n" NOR);
        switch (random(8))
        {
        case 0:
            message_vision(HIW "$N" HIW "一言不发，对$n" HIW "发动了攻击。\n" NOR, gob, me);
            break;
        case 1:
...
void want_kill(object ob)
{
    ...
    if (! arrayp(want_kills))
        want_kills = ({ });

    ob_id = ob->query("id");

    if (is_killing(ob_id) || is_want_kill(ob_id))
        return;
...
////这个函数也一直在刷新（感觉是每秒刷新），刷新的原因是被attack()调用。
void clean_up_enemy()
{
    int i;
...
//这个函数一直在刷新。enemy列表一出戏敌人就执行if语句里的内容。
int attack()
{
    object opponent;
